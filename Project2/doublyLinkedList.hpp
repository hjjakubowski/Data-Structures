...
public:
    ...
    Elem& operator[](int index);               // Non-const version
    const Elem& operator[](int index) const;   // Const version
...
void DLinkedList<Elem>::add(const Elem& e, int pos) {
    ...
    if (pos == 0) {
        ...
        current->next->prev = newNode;         // Current's next's prev points to new node
...
template <typename Elem>
Elem& DLinkedList<Elem>::operator[](int index) {
...
template <typename Elem>
const Elem& DLinkedList<Elem>::operator[](int index) const {
    if (index < 0 || index >= getSize()) {
        throw std::out_of_range("Index out of range");
    }
